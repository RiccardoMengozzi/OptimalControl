...
def optimal_trajectory(xx_ref, uu_ref, initial_xx, dt, max_iters):
    ...
    for kk in range(max_iters-1): #Start of algorithm for cost minimization
        ...
        for tt in range(TT-1):
            xx[:,tt+1, kk], At[:, :, tt], Bt[:, :, tt] = dynamics.nominal_dynamics(xx[:, tt, kk], uu[:, tt, kk], dt) #Evaluation of state-input trajectory and matrix linear approximation at iteration kk
            if tt > 0:
    ...
    # Compute descent direction using cvxpy
        ...
        for ii in range(armijo_maxiters):
            ...
            for tt in range(TT-1):
                ...
                xx_temp[:,tt+1] = dynamics.nominal_dynamics(xx_temp[:,tt], uu_temp[:,tt], dt)[0]
