...
middle_xx = np.array([1800, 0*DEG_TO_RAD, 7*DEG_TO_RAD, 0])
...
if not step_reference:
    if double_transition:
      ...
      if cubic:
        coefficients1.append([cubic_polynomial_coefficients(t01, initial_xx[i], tf1, middle_xx[i]) for i in range(ns)])
        coefficients1 = np.array(coefficients1).reshape((ns,4))

        transition_values1.append([cubic_polynomial(time_values1, coefficients1[i]) for i in range(ns)])
        transition_values1 = np.array(transition_values1).reshape((4,tf1-t01+1))

        coefficients2.append([cubic_polynomial_coefficients(t02, middle_xx[i], tf2, final_xx[i]) for i in range(ns)])
        coefficients2 = np.array(coefficients2).reshape((ns,4))

        transition_values2.append([cubic_polynomial(time_values2, coefficients2[i]) for i in range(ns)])
        transition_values2 = np.array(transition_values2).reshape((4,tf2-t02+1))

      if quintic:
        coefficients1.append([quintic_polynomial_coefficients(t01, initial_xx[i], tf1, middle_xx[i]) for i in range(ns)])
        coefficients1 = np.array(coefficients1).reshape((ns,6))

        transition_values1.append([quintic_polynomial(time_values1, coefficients1[i]) for i in range(ns)])
        transition_values1 = np.array(transition_values1).reshape((4,tf1-t01+1))

        coefficients2.append([quintic_polynomial_coefficients(t02, middle_xx[i], tf2, final_xx[i]) for i in range(ns)])
        coefficients2 = np.array(coefficients2).reshape((ns,6))

        transition_values2.append([quintic_polynomial(time_values2, coefficients2[i]) for i in range(ns)])
        transition_values2 = np.array(transition_values2).reshape((4,tf2-t02+1))
      ...
      if cubic:
        coefficients.append([cubic_polynomial_coefficients(t0, initial_xx[i], tf, final_xx[i]) for i in range(ns)])
      ...
      if quintic:
        coefficients.append([quintic_polynomial_coefficients(t0, initial_xx[i], tf, final_xx[i]) for i in range(ns)])
